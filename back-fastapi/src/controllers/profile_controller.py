...
async def get_profile(res: Response, access_token: str = Cookie(None)) -> Optional[dict]:
...
async def generate_profile(
    res: Response, data: GenerateProfileDTO = Depends(validate_profile_generate), access_token: str = Cookie(None)
) -> Optional[dict]:
    if access_token is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        payload = auth_service.decode_token(access_token)
        profile = await profile_service.generate_profile(payload["account_id"], data)
        return None
